...
-- Fix trigger to handle tables without statut column
...
BEGIN
  ...
  -- Prefer responsable_user_id if available for user resolution
  BEGIN
  ...
  -- Fallback: resolve user_id from acteur_id
  ...
  -- CHECK STATUS CHANGE (safe: use to_jsonb to check if statut column exists)
  IF TG_OP = 'UPDATE' THEN
